...
def _log_error(chart_name, column, error):
    logger.error("[EDA ERROR] %s | Column: %s | %s", chart_name, column or 'N/A', str(error))
...
def analyze_timeseries(df, date_col=None):
    if date_col is None:
        ...
        if date_col is None:
            for col in df.columns:
                try:
                    pd.to_datetime(df[col].dropna().head(100), utc=True)
                    date_col = col
                    break
                except Exception as e:
...
def generate_charts(df, output_dir, selected_charts=None):
    ...
    has_missing = bool(compute_missing_analysis(df)['columns'])
    all_charts = selected_charts or ['histogram', 'boxplot', 'density', 'correlation', 'missing',
                                     'missing_heatmap', 'bar', 'pie', 'trend']

    app_num = len(numeric)
    cat_num = len(categorical)
    date_num = len(date_cols)

    applicability = {
        'histogram': app_num > 0,
        'boxplot': app_num > 0,
        'density': app_num >= 2,
        'correlation': app_num >= 2,
        'missing': has_missing,
        'missing_heatmap': has_missing,
        'bar': cat_num > 0,
        'pie': cat_num > 0,
        'trend': date_num > 0 and app_num > 0,
        'rolling_avg': date_num > 0 and app_num > 0,
    }
    charts['_applicable'] = applicability

    if 'histogram' in all_charts:
        charts['histograms'] = []
        for col in numeric[:6]:
            chart_data, img = _try_generate('histogram', col, output_dir,
                                            histogram, df, col, output_dir)
            if chart_data:
                charts['histograms'].append({'column': col, 'plotly': chart_data, 'image': img})
    if 'boxplot' in all_charts:
        charts['boxplots'] = []
        for col in numeric[:6]:
            chart_data, img = _try_generate('boxplot', col, output_dir,
                                            boxplot, df, col, output_dir)
            if chart_data:
                charts['boxplots'].append({'column': col, 'plotly': chart_data, 'image': img})
    if 'density' in all_charts:
        charts['density'] = []
        for col in numeric[:6]:
            chart_data, img = _try_generate('density', col, output_dir,
                                            density_plot, df, col, output_dir)
            if chart_data:
                charts['density'].append({'column': col, 'plotly': chart_data, 'image': img})
    if 'correlation' in all_charts:
        chart_data, img = _try_generate('correlation', 'all', output_dir,
    ...
    if 'missing' in all_charts:
        missing_cols = compute_missing_analysis(df)['columns']
        if missing_cols:
            chart_data, img = _try_generate('missing', 'all', output_dir,
                                            missing_values_chart, missing_cols, output_dir)
            charts['missing'] = {'plotly': chart_data, 'image': img} if chart_data else None
    if 'missing_heatmap' in all_charts:
        chart_data, img = _try_generate('missing_heatmap', 'all', output_dir,
                                        missing_heatmap, df, output_dir)
    ...
    if 'bar' in all_charts:
        ...
        for col in categorical[:3]:
            try:
                ...
                freq = {}
            chart_data, img = _try_generate('bar', col, output_dir,
                                            bar_chart, freq, f'Top Values — {col}', output_dir, f'bar_{col}')
            if chart_data:
    ...
    if 'pie' in all_charts:
        ...
        for col in categorical[:3]:
            try:
                ...
                freq = {}
            chart_data, img = _try_generate('pie', col, output_dir,
                                            pie_chart, freq, f'Distribution — {col}', output_dir, f'pie_{col}')
            if chart_data:
    ...
    if 'trend' in all_charts:
        if date_cols and numeric:
            ...
            value_col = numeric[0]
            chart_data, img = _try_generate('trend_chart', value_col, output_dir,
                                            trend_chart, df, date_col, value_col, output_dir)
            charts['trend'] = {'plotly': chart_data, 'image': img} if chart_data else None
            ra_data, ra_img = _try_generate('rolling_average_chart', value_col, output_dir,
                                            rolling_average_chart, df, date_col, value_col, 7, output_dir)
    ...
    total_charts = sum(len(v) for v in charts.values() if isinstance(v, list)) + \
                   sum(1 for v in charts.values() if isinstance(v, dict) and v is not None and v.get('plotly'))
