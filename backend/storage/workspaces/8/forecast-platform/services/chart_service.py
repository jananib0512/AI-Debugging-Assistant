...
def _log_error(chart_name, column, error):
    logger.error("[CHART ERROR] %s | Column: %s | %s", chart_name, column or 'N/A', str(error))
...
def missing_heatmap(df, output_dir):
    ...
    z = plot_data.astype(float).to_numpy().T
...
def _prepare_datetime(series, column):
    """Detect and convert a series to UTC-normalized datetime. Returns pd.Series or None."""
    if series is None or len(series) == 0:
        return None
    if pd.api.types.is_datetime64_any_dtype(series):
        return series.dropna()
    try:
        converted = pd.to_datetime(series, errors='coerce', utc=True).dropna()
        if len(converted) == 0:
...
def _try_generate(chart_type, column, output_dir, func, *args, **kwargs):
    """Reusable chart generation wrapper with consistent error handling.

    Returns (plotly_json, image_path) on success, (None, None) on failure.
    """
    try:
        ...
        _log_error(chart_type, column or 'unknown', str(e))
    return None, None
