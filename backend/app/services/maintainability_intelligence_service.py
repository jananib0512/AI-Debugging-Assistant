...
class MaintainabilityIntelligenceService:
    def __init__(self, db: Session = Depends(get_db)):
        self._project_repo = ProjectRepository(db)

    def analyze(self, user_id: int, project_id: int) -> MaintainabilityIntelligenceResponse:
        ...
        engine = MaintainabilityIntelligenceEngine()
        ...
        score = MaintainabilityScore(**result["maintainability_score"])
        smells = [CodeSmellItem(**s) for s in result.get("code_smells", [])]
        tech_debt = TechnicalDebtEstimate(**result["technical_debt"])
        module_health = [ModuleHealthScore(**m) for m in result.get("module_health", [])]
        summary = MaintainabilitySummary(**result["summary"])

        return MaintainabilityIntelligenceResponse(
