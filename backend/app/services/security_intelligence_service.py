...
class SecurityIntelligenceService:
    ...
    def analyze(self, user_id: int, project_id: int) -> SecurityIntelligenceResponse:
        ...
        score = SecurityScore(**result["security_score"])
        findings = [SecurityFinding(**f) for f in result.get("findings", [])]
        dep_issues = [DependencySecurity(**d) for d in result.get("dependency_issues", [])]
