...
class RuntimeDetectionService:
    ...
    def analyze(self, user_id: int, project_id: int) -> RuntimeAnalysisResponse:
        ...
        engine = RuntimeDetectionEngine()
        result = engine.analyze(workspace_path)

        results = [RuntimeAnalysisResult(**r) for r in result.get("results", [])]
        ...
        if result["status"] == "unavailable":
            ...
            summary = f"Runtime analysis completed. No runtime risks detected. The project has proper error handling and resource management."
        else:
            parts = [f"Runtime analysis completed. {result['total_errors']} runtime risks detected."]
            ...
            parts.append("The project has runtime risks that should be addressed before deployment.")
            summary = " ".join(parts)
