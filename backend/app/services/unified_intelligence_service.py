from datetime import datetime, timezone
from pathlib import Path

from fastapi import Depends, HTTPException, status
from sqlalchemy.orm import Session

from app.core.database import get_db
from app.core.storage import get_workspace_path
from app.repositories.project_repository import ProjectRepository
from app.repositories.unified_intelligence_engine import UnifiedIntelligenceEngine
from app.schemas.project_analyzer import (
    ExecutiveSummary,
    GlobalInsight,
    GlobalSearchResult,
    HealthMapModule,
    KnowledgeHubItem,
    ProjectTimelineStage,
    UnifiedHealthMetrics,
    UnifiedIntelligenceResponse,
    UnifiedProjectScore,
)


class UnifiedIntelligenceService:
    def __init__(self, db: Session = Depends(get_db)):
        self._project_repo = ProjectRepository(db)

    def analyze(self, user_id: int, project_id: int) -> UnifiedIntelligenceResponse:
        project = self._project_repo.get_by_id(project_id)
        if not project:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found")
        if project.user_id != user_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
        if project.extraction_status != "extracted":
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                detail="Project workspace has not been extracted.")

        workspace_path = get_workspace_path(project_id)
        if not workspace_path.exists():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Workspace directory not found.")

        data = self._aggregate_all_engines(workspace_path)
        engine = UnifiedIntelligenceEngine()
        result = engine.analyze(
            workspace_path,
            project_analysis=data.get("project_analysis"),
            code_intel=data.get("code_intel"),
            code_quality=data.get("code_quality"),
            file_analysis=data.get("file_analysis"),
            func_class=data.get("func_class"),
            dep_analysis=data.get("dep_analysis"),
            call_graph=data.get("call_graph"),
            semantic=data.get("semantic"),
            config_intel=data.get("config_intel"),
            recommendations=data.get("recommendations"),
        )

        health = UnifiedHealthMetrics(**result["health"])
        scores = UnifiedProjectScore(**result["scores"])
        insights = [GlobalInsight(**i) for i in result.get("insights", [])]
        exec_summary = ExecutiveSummary(**result["executive_summary"])
        knowledge_hub = [KnowledgeHubItem(**k) for k in result.get("knowledge_hub", [])]
        health_map = [HealthMapModule(**h) for h in result.get("health_map", [])]
        timeline = [ProjectTimelineStage(**t) for t in result.get("timeline", [])]

        return UnifiedIntelligenceResponse(
            health=health,
            scores=scores,
            insights=insights,
            executive_summary=exec_summary,
            knowledge_hub=knowledge_hub,
            health_map=health_map,
            timeline=timeline,
            search_results={},
            analyzed_at=datetime.now(timezone.utc),
        )

    def _aggregate_all_engines(self, workspace_path: Path) -> dict:
        result = {}

        try:
            from app.repositories.project_intelligence_engine import ProjectIntelligenceEngine
            pi_engine = ProjectIntelligenceEngine()
            result["project_analysis"] = pi_engine.analyze(workspace_path)
        except Exception:
            result["project_analysis"] = None

        try:
            from app.repositories.source_code_intelligence_engine import SourceCodeIntelligenceEngine
            sc_engine = SourceCodeIntelligenceEngine()
            sc_raw = sc_engine.analyze(workspace_path)
            result["code_intel"] = sc_raw
        except Exception:
            result["code_intel"] = None

        try:
            from app.repositories.code_quality_engine import CodeQualityEngine
            cq_engine = CodeQualityEngine()
            result["code_quality"] = cq_engine.analyze(workspace_path)
        except Exception:
            result["code_quality"] = None

        try:
            from app.repositories.file_intelligence_engine import FileIntelligenceEngine
            fi_engine = FileIntelligenceEngine()
            result["file_analysis"] = fi_engine.analyze(workspace_path)
        except Exception:
            result["file_analysis"] = None

        try:
            from app.repositories.function_class_intelligence_engine import FunctionClassIntelligenceEngine
            fc_engine = FunctionClassIntelligenceEngine()
            result["func_class"] = fc_engine.analyze(workspace_path)
        except Exception:
            result["func_class"] = None

        try:
            from app.repositories.import_dependency_engine import ImportDependencyEngine
            id_engine = ImportDependencyEngine()
            result["dep_analysis"] = id_engine.analyze(workspace_path)
        except Exception:
            result["dep_analysis"] = None

        try:
            from app.repositories.call_graph_intelligence_engine import CallGraphIntelligenceEngine
            cg_engine = CallGraphIntelligenceEngine()
            result["call_graph"] = cg_engine.analyze(workspace_path)
        except Exception:
            result["call_graph"] = None

        try:
            from app.repositories.semantic_intelligence_engine import SemanticCodeIntelligenceEngine
            sm_engine = SemanticCodeIntelligenceEngine()
            result["semantic"] = sm_engine.analyze(workspace_path)
        except Exception:
            result["semantic"] = None

        try:
            from app.repositories.configuration_intelligence_engine import ConfigurationIntelligenceEngine
            ci_engine = ConfigurationIntelligenceEngine()
            result["config_intel"] = ci_engine.analyze(workspace_path)
        except Exception:
            result["config_intel"] = None

        try:
            from app.repositories.project_insights_engine import ProjectInsightsEngine
            pi_engine = ProjectInsightsEngine()
            project_data = result.get("project_analysis", {})
            code_intel = result.get("code_intel", {})
            result["recommendations"] = pi_engine.analyze(workspace_path, project_data, code_intel)
        except Exception:
            result["recommendations"] = None

        return result
