...
class SyntaxDetectionService:
    def __init__(self, db: Session = Depends(get_db)):
        self._project_repo = ProjectRepository(db)

    def detect(self, user_id: int, project_id: int) -> SyntaxDetectionResponse:
        ...
        engine = SyntaxDetectionEngine()
        result = engine.analyze(workspace_path)

        results = [SyntaxDetectionResult(**r) for r in result.get("results", [])]
        language = project.language or ", ".join(result.get("scanned_languages", []))

        if result["status"] == "unavailable":
            summary = f"The project workspace for \"{project.project_name}\" is not available. Please ensure the project has been uploaded and extracted."
        elif result["total_errors"] == 0:
            summary = f"Syntax analysis completed. No syntax errors detected. The project appears to be well-formed."
        else:
            parts = [f"Syntax analysis completed. {result['total_errors']} issues detected."]
            ...
            parts.append("The project cannot compile successfully until these issues are resolved.")
            summary = " ".join(parts)

        return SyntaxDetectionResponse(
